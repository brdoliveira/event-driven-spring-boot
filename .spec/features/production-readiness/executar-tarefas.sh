#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano production-readiness` em 2026-08-24 02:32
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo production-readiness
set -u
set -o pipefail

RUN_ID='event-driven-spring-boot-production-readiness-mt6mf59e'
FEATURE='production-readiness'
BASE_BRANCH='spec/production-readiness'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/production-readiness/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/event-driven-spring-boot-production-readiness-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/event-driven-spring-boot-production-readiness"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/production-readiness/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/production-readiness-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-001' 'Você executa UMA tarefa da feature "production-readiness" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/production-readiness/spec.md, .spec/features/production-readiness/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-001 — "Tornar o build reproduzível e contínuo"
  critérios/refs: AC-001 (Clone limpo compila todos os módulos)
  arquivos permitidos (e seus testes): pom.xml, mvnw, mvnw.cmd, .mvn/wrapper/maven-wrapper.properties, .github/workflows/ci.yml, README.md, onpspec.config.json, .spec/constituicao.md
  mensagem de commit: "T-001 production-readiness: Tornar o build reproduzível e contínuo"

Regras inegociáveis:
- Todo critério de aceite referenciado vira teste com @spec:AC-xxx no título.
- NUNCA enfraqueça, pule (skip/todo) ou apague um teste para passar — teste pulado não é prova e o audit acusa.
- Rode os testes localmente com `mvnw.cmd -q test` até passarem.
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/production-readiness-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/production-readiness-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-002' 'Você executa UMA tarefa da feature "production-readiness" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/production-readiness/spec.md, .spec/features/production-readiness/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-002 — "Corrigir e testar o contrato de criação de produtos"
  critérios/refs: AC-002 (Produto válido é criado), AC-003 (Quantidade ausente ou inválida é rejeitada)
  arquivos permitidos (e seus testes): products-service/pom.xml, products-service/src/main/java/com/appsdeveloperblog/products/dto/ProductCreationRequest.java, products-service/src/test/java/com/appsdeveloperblog/products/web/controller/ProductsControllerTest.java
  mensagem de commit: "T-002 production-readiness: Corrigir e testar o contrato de criação de produtos"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/production-readiness-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/production-readiness-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-003' 'Você executa UMA tarefa da feature "production-readiness" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/production-readiness/spec.md, .spec/features/production-readiness/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-003 — "Externalizar e limitar a integração de pagamentos"
  critérios/refs: AC-004 (Chamada remota tem limites de espera configuráveis), AC-005 (Cartão de demonstração vem da configuração)
  arquivos permitidos (e seus testes): payments-service/src/main/java/com/appsdeveloperblog/payments/config/PaymentProcessorProperties.java, payments-service/src/main/java/com/appsdeveloperblog/payments/config/ApplicationConfig.java, payments-service/src/main/java/com/appsdeveloperblog/payments/service/PaymentServiceImpl.java, payments-service/src/main/resources/application.properties, payments-service/src/test/java/com/appsdeveloperblog/payments/config/ApplicationConfigTest.java, payments-service/src/test/java/com/appsdeveloperblog/payments/service/PaymentServiceImplTest.java
  mensagem de commit: "T-003 production-readiness: Externalizar e limitar a integração de pagamentos"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' high
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/production-readiness-faixa-3' "$WT" "$st" || return 1
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/production-readiness-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-004' 'Você executa UMA tarefa da feature "production-readiness" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/production-readiness/spec.md, .spec/features/production-readiness/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-004 — "Provar o caminho compensatório da saga"
  critérios/refs: AC-006 (Falha no pagamento inicia devolução do estoque), AC-007 (Estoque devolvido rejeita o pedido)
  arquivos permitidos (e seus testes): orders-service/src/test/java/com/appsdeveloperblog/orders/saga/OrderSagaTest.java
  mensagem de commit: "T-004 production-readiness: Provar o caminho compensatório da saga"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/production-readiness-faixa-4' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/production-readiness/executar-tarefas.sh --gate"
      exit 0
    fi
