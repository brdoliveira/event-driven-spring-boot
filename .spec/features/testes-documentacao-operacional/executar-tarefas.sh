#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano testes-documentacao-operacional` em 2026-08-25 01:07
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo testes-documentacao-operacional
set -u
set -o pipefail

RUN_ID='event-driven-spring-boot-testes-documentacao-operacional-mt7ytkrt'
FEATURE='testes-documentacao-operacional'
BASE_BRANCH='spec/testes-documentacao-operacional'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/testes-documentacao-operacional/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/event-driven-spring-boot-testes-documentacao-operacional-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/event-driven-spring-boot-testes-documentacao-operacional"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/testes-documentacao-operacional/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/testes-documentacao-operacional-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-005' 'Você executa UMA tarefa da feature "testes-documentacao-operacional" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/testes-documentacao-operacional/spec.md, .spec/features/testes-documentacao-operacional/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-005 — "Testar os contratos HTTP ainda descobertos"
  critérios/refs: AC-008 (Pedido válido é aceito), AC-009 (Pedido inválido é rejeitado antes do serviço), AC-010 (Processador de cartão valida sua entrada)
  arquivos permitidos (e seus testes): orders-service/src/test/java/com/appsdeveloperblog/orders/web/controller/OrdersControllerTest.java, credit-card-processor-service/pom.xml, credit-card-processor-service/src/test/java/com/appsdeveloperblog/ccps/web/controller/CreditCardProcessorControllerTest.java
  mensagem de commit: "T-005 testes-documentacao-operacional: Testar os contratos HTTP ainda descobertos"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/testes-documentacao-operacional-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/testes-documentacao-operacional-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-006' 'Você executa UMA tarefa da feature "testes-documentacao-operacional" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/testes-documentacao-operacional/spec.md, .spec/features/testes-documentacao-operacional/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-006 — "Testar os handlers de produto e pagamento"
  critérios/refs: AC-011 (Reserva de produto publica o resultado correspondente), AC-012 (Cancelamento de reserva confirma a compensação), AC-013 (Processamento de pagamento publica o resultado correspondente)
  arquivos permitidos (e seus testes): products-service/src/test/java/com/appsdeveloperblog/products/service/handler/ProductCommandsHandlerTest.java, payments-service/src/test/java/com/appsdeveloperblog/payments/service/handler/PaymentsCommandsHandlerTest.java
  mensagem de commit: "T-006 testes-documentacao-operacional: Testar os handlers de produto e pagamento"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' high
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/testes-documentacao-operacional-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/testes-documentacao-operacional-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-007' 'Você executa UMA tarefa da feature "testes-documentacao-operacional" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/testes-documentacao-operacional/spec.md, .spec/features/testes-documentacao-operacional/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-007 — "Criar guia operacional e relatórios de cobertura"
  critérios/refs: AC-014 (README permite executar e demonstrar o projeto), AC-015 (Build gera relatório de cobertura)
  arquivos permitidos (e seus testes): README.md, pom.xml, .github/workflows/ci.yml, core/src/test/java/com/appsdeveloperblog/core/DocumentationCompletenessTest.java, core/src/test/java/com/appsdeveloperblog/core/BuildQualityConfigurationTest.java, onpspec.config.json
  mensagem de commit: "T-007 testes-documentacao-operacional: Criar guia operacional e relatórios de cobertura"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/testes-documentacao-operacional-faixa-3' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/testes-documentacao-operacional/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-005"
  echo "  faixa-2  onda 1  T-006"
  echo "  faixa-3  onda 1  T-007"
  echo
